...
class ApiKeyTest < ActiveSupport::TestCase
  ...
  test "encrypts and decrypts google_maps_key" do
    api_key = users(:one).api_keys.create!(google_maps_key: "AIzaSyNewTestKey999")
    api_key.reload
    assert_equal "AIzaSyNewTestKey999", api_key.google_maps_key
  end

  test "default label is Default" do
    api_key = ApiKey.new(user: users(:one), google_maps_key: "AIzaSyTest")
    assert_equal "Default", api_key.label
  end

  test "valid with all required attributes" do
    api_key = ApiKey.new(user: users(:one), google_maps_key: "AIzaSyTest123")
